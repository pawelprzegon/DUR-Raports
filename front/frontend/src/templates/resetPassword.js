...
import {
  checkAuthResetPassword,
  callApiPost,
} from '../features/endpoints/endpoints.js';
...
export default class extends AbstractView {
  ...
  async getData() {
    let [response, status] = await checkAuthResetPassword(
      this.api_url_auth,
      this.token
    );
    ...
    if (
      ...
      alerts(
        status,
        response.detail + ': wygeneruj nowy link do zresetowania hasła',
        'alert-red'
      );
    }
  }
  ...
  async show() {
    ...
    data.forEach((each) => {
      elemBox = document.createElement('div');
      elemBox.classList.add('input-control');
      elemBox.id = 'div-' + each;
      ...
      elemBox.appendChild(this.elemLabel);
      elemBox.appendChild(this.elemText);
      elemBox.appendChild(this.Err);
      this.formField.appendChild(elemBox);
    });
